...
Streamcore.runStream = function (cameraid, camera, options, destination, key, thumbnail)
{
    ...
    var pipeSource = function (source) {
        ...
        if (ffmpeg.available && (options.width > 0 || options.height > 0 || camera.rotate || (options.encoder && options.encoder !== 'jpeg'))) {
            ...
            encoder = ffmpeg.getEncoder(options, camera.rotate);
            ...
            encoder = ffmpeg.makeDuplex(consumer, limiter);
        }
...
Streamcore.stopStreaming = function (cameraid, options, key) {
    ...
    if (sources[cameraid] && encoders[key]) {
        sources[cameraid].unpipe(encoders[key]);
        encoders[key].unpipe(destinations[key]);
        encoders[key].end();
    } else if (sources[cameraid] && destinations[key]) {
