...
ffmpeg.getEncoder = function (options, rotation) {
    ...
    case 'vp8':
        ...
        args.push('-vf', vf);
        args.push('-f', 'webm', '-c:v', 'libvpx', '-b:v', '500k', '-crf', 10, '-static-thresh', 100, '-keyint_min', options.framerate * 5, '-r', options.framerate, '-quality', 'realtime', '-');
        break;
    ...
    case 'vp9':
        ...
        vf += ",setpts='(RTCTIME - RTCSTART) / (TB * 1000000)'";
        args.push('-vf', vf);
        args.push('-f', 'webm', '-c:v', 'libvpx-vp9', '-b:v', '500k', '-crf', 10, '-static-thresh', 100, '-keyint_min', options.framerate * 5, '-r', options.framerate, '-quality', 'realtime', '-');
        break;
