...
ffmpeg.getEncoder = function (options, rotation) {
    ...
    case 'vp8':
        ...
        args.push('-f', 'webm', '-c:v', 'libvpx', '-b:v', '800k', '-keyint_min', 50, '-r', 25, '-quality', 'realtime', '-cpu-used', '-5', '-crf', 33, '-');
        break;

    case 'vp9':
        // Intended for Android 4.4+
        // Currently very slow compared to vp8
        args.push('-f', 'webm', '-c:v', 'libvpx-vp9', '-b:v', '800k', '-keyint_min', 50, '-r', 25, '-quality', 'realtime', '-cpu-used', '-5', '-crf', 33, '-');
        break;
