...
function reloadSchedule(cb)
{
    ...
    for (var i in schedule) {
        ...
        if (!s.hasOwnProperty('dateTime') || s.dateTime === null) {
        ...
        var datetime = s.dateTime.timestamp ? new Date(s.dateTime.timestamp) : s.dateTime;

        var j = scheduler.scheduleJob(datetime, jobRunner(s));
...
function jobRunner(s)
{
    return function () {
        ...
        if (s.dateTime.year || s.dateTime.timestamp) {
            removeJob(s.id);
        }
