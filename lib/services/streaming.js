...
function startPlayback(recordingid, cb)
{
    var playbackid = require('node-uuid').v4();

    var filename = require('path').join(getVideoDirectory(), recordingid + '.webm');

    var source = playbacks[playbackid] = ffmpeg.playRecording(filename, options);

    var sio = getSocketIOStream(recordingid, options);
...
function endPlayback(playbackid, cb)
{
